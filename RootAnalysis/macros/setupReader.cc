...
TMVA::Reader* setupReader(string xmlFile, ReaderData &rd) {
  ...
  for (unsigned int i = 0; i < allLines.size(); ++i) {
	...
	if (stype == "flsxy") {
	  //	  cout << "  adding flsxy" << endl;
	  reader->AddVariable( "flsxy", &rd.flsxy);
	}
	if (stype == "alpha") {
...
// ----------------------------------------------------------------------
TMVA::Reader* setupReader(string xmlFile, ReaderData &rd, presel &a) {
  TMVA::Reader *reader = new TMVA::Reader( "!Color:Silent" );
  ...
  //  cout << "  read " << allLines.size() << " lines " << endl;
  for (unsigned int i = 0; i < allLines.size(); ++i) {
    // -- parse and add preselection
    if (string::npos != allLines[i].find("Info name=\"Preselection:")) {
      m1 = allLines[i].find("Preselection:")+11;
      m2 = allLines[i].find("\" value=\"");
      stype = allLines[i].substr(m1+2, m2-m1-2);
      m1 = m2+9;
      m2 = allLines[i].find("/>") - 1;
      sval = allLines[i].substr(m1, m2-m1);
      a.setCut(stype, atof(sval.c_str()));
    }
    // -- parse and add variables
    if (string::npos != allLines[i].find("Variables NVar")) {
      m1 = allLines[i].find("=\"");
