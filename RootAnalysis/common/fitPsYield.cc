...
void fitPsYield::fit0_Bs2JpsiPhi(psd *res, int limitpars, string pdfprefix) {
  ...
  if (limitpars < 0) {
    ...
    f1->SetParameter(3, sBp);    f1->SetParLimits(3, 0.010, 0.040);
...
//           = 0: unconstrained fit, based on starting values of prior (limitpars < 0) call
//           > 0: constrain parameters within limitpars*sigma of prior (limitpars < 0) call
void fitPsYield::fit0_Bu2JpsiKpOld(psd *res, int limitpars, string pdfprefix) {
  TH1D *h = res->fH1;
  setTitles(h, "#it{m}_{#it{#mu#mu}K} #it{[GeV]}", Form("#it{Candidates/(%4.3f GeV)}", h->GetBinWidth(1)), 0.05, 1.1, 1.9);
  if (0 == fData.size()) return;
  fSummary.clear();
  ...
  fpIF->fName = "fit";
  TF1 *f1 = fpIF->expoErrgauss2Landau(h);
  fpIF->fName = "comp";
  TF1 *fg  = fpIF->gauss2c(0., 100.);
  fg->SetLineColor(kBlue+1);
  TF1 *fe = fpIF->err2(0., 100.);
  ...
  if (limitpars < 0) {
    ...
    f1->SetParameter(4, 5*sBp);  f1->SetParLimits(4, 0.050, 0.10);
    // 5 6 7 for the 3rd gaussian


    f1->SetParameter(8, p0);      f1->SetParLimits(8, 0., -1.);
    f1->SetParameter(9, p1);      f1->SetParLimits(9, 0., -1.);
    f1->SetParameter(10, stepBp); f1->SetParLimits(10, stepBp - 2.*sBp, stepBp + 2.*sBp);
    f1->SetParameter(11, 2.*sBp); f1->SetParLimits(11, 2.*sBp - sBp, 2.*sBp + 1.5*sBp);
    f1->SetParameter(12, errN);   f1->SetParLimits(12, 0., h->GetMaximum());
  } else {
    for (int ipar = 0; ipar < f1->GetNpar(); ++ipar) {
      f1->ReleaseParameter(ipar);
    }
  ...
  for (int ipar = 0; ipar < fe->GetNpar(); ++ipar) fe->SetParameter(ipar, f1->GetParameter(ipar+10));
  for (int ipar = 0; ipar < fp->GetNpar(); ++ipar) fp->SetParameter(ipar, f1->GetParameter(ipar+8));
  fg->Draw("same");
  f1->Draw("same");
  fe->Draw("same");
  fp->Draw("same");
